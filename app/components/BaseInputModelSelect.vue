<template>
  <div class="relative">
    <div
      class="flex w-[15rem] items-center space-x-2 rounded-full border border-zinc-300 bg-white p-2 transition ease-in"
      :class="{
        'shadow-lg': toggled,
    ...
    >
      <img
        ...
        class="pointer-events-none h-14 w-14 flex-none rounded-full object-cover transition"
      />
      <div class="w-full overflow-hidden pr-4 transition">
        <p class="text-sm font-medium text-zinc-900 transition">
          {{ currentModel?.label }}
        </p>
        <p
    ...
    <Transition>
      <div
        v-if="toggled"
        ref="menu"
        class="absolute left-1/2 top-20 z-50 grid w-[20rem] -translate-x-1/2 transform grid-cols-2 gap-4 rounded-lg border border-zinc-300 bg-white px-4 py-2 shadow-lg sm:w-[30rem] sm:grid-cols-3"
      >
        <div
        ...
        >
          <nuxt-img
            :src="model.picture"
            class="pointer-events-none h-14 w-14 rounded-full object-cover"
          />
          <div>
